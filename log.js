    ...
    storeD.addEventListener("click", function () {
      ...
      deletDivHElements.forEach((deletDivH) => {
        deletDivH.addEventListener("click", function () {
          auth.onAuthStateChanged((user) => {
            if (user) {
              //Deleting innerText from Firestore
              const collectionRef = doc(firestore, "div_contents", user.uid);

              const innerTextDivH = deletDivH.innerText.trim();
              const pathDoc = collectionRef;

              //Decrementation variables:
              getDoc(pathDoc)
                .then((docSnapshot) => {
                  if (docSnapshot.exists()) {
                    const data = docSnapshot.data();
                  let spentTillNow = data.spentTillNow || 0;

                  const updateData = totalSum;
                  spentTillNow = totalSum;
                  const spending = document.getElementById("spendings");
                  const currency = document.getElementById("currency-select");
                  spending.innerHTML = "Till now you have spent: <br> " + totalSum + " " + currency.value;
                  ...
                  updateDoc(pathDoc, { spentTillNow: updateData })
                    ...
                    if (documentData.hasOwnProperty("content") && Array.isArray(documentData.content)) {
                      const divHInnerText = innerTextDivH;
      ...
      //Decrementation of the variable spentTillNow: 
      const divHArray = document.querySelectorAll(".expenses--div");
      const deletButtonList = document.querySelectorAll(".button-data");
      const deletButton = document.querySelector(".button-data")
      deletButton.addEventListener("click", () => {
        auth.onAuthStateChanged((user) => {
          if (user) {
            const pathDoc = doc(firestore, "div_contents", user.uid);

            const expensesSpans = document.querySelectorAll(".expensesSpan");
            let totalSum = 0;

            expensesSpans.forEach((span) => {
              const value = parseFloat(span.innerText); // Convert inner text to a number
              if (!isNaN(value)) {
                totalSum += value;
              }
            });
            ...
            getDoc(pathDoc)
              .then((docSnapshot) => {
                if (docSnapshot.exists()) {
                  ...
                  //valuSpan style based on daily goal
                  const dailyGoal = document.getElementById("goal");
                  const maxSpending = divHArray.length * goal.value;
                  
                  if(maxSpending > totalSum){
                    valueSpan.style.color = "#34C759"
                  } else{
                    valueSpan.style.color = "#FF3B30"
                  }
                  ...
                  //Function loading bar loading..... Till goal is reached
                  const exDivLenght = document.querySelectorAll(".expenses--div");
                  const maxSpent = exDivLenght.length * goal.value;
                  let savedUp = maxSpent - valueSpan.innerText;
                  if(savedUp < 0){
                    savedUp = 0;
                  }


                  const percOfFill = (100 / savingGoal) * savedUp;
                  fill.style.width = percOfFill + "%";



                  counter.innerText = Math.round(percOfFill) + "%";


                  
                  // Append valueSpan to the spending element
                  spending.appendChild(valueSpan);
                  valueSpan.innerText = totalSum;
                  
                  // Update inner HTML of spending
                  spending.innerHTML = "Till now you have spent: <br>" + valueSpan.outerHTML + " " + currency.value;
            ...
            .then((docSnapshot) => {
              if (docSnapshot.exists()) {
                ...
                dataArray.forEach((currentString) => {
                  const currenccV = document.getElementById("currency-select").value;
                  const divH = document.createElement("div");
                  divH.className = "expenses--div";
                  divH.id = "2";


                  //Checking if the strings inclueds currenccyV value
                  if(currentString.includes("USD")){
                    currentString = currentString.replace("USD", "")
                  }
                  if(currentString.includes("EUR")){
                    currentString = currentString.replace("EUR", "")
                  }
                  if(currentString.includes("GBP")){
                    currentString = currentString.replace("GBP", "")
                  }
                  if(currentString.includes("JPY")){
                    currentString = currentString.replace("JPY", "")
                  }
                  if(currentString.includes("CAD")){
                    currentString = currentString.replace("CAD", "")
                  }
                 


                  divH.innerText = currentString + " " + currenccV;
                            ...
                            if (indexToRemove !== -1) {
                              ...
                              updateDoc(pathDoc, { content: documentData.content })
                                .then(() => {
                                  // Element removed successfully from Firestore, now remove from DOM
                                  divH.remove();
                                  console.log("Element removed successfully from Firestore and DOM!");
                                })
                                .catch((error) => {
                                  console.log("Error updating document:", error);
                                });
